...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="File where to write the namespace:name of the repositories that failed to patch.",
    )
...
def main() -> None:
    ...
    for patch in patches:
        try:
            log.info(
                "re-checking if %s:%s hasn't been updated in the meantime",
                patch.repo_namespace,
                patch.repo_name,
            )
            new_patch = refresh_patch(patch, namespace_to_workspace_map)
            if new_patch is None:
                continue
            apply_patch(new_patch)
        except subprocess.CalledProcessError:
            log.warning("failed to patch %s:%s", patch.repo_namespace, patch.repo_name)
            failed_to_patch.append(f"{patch.repo_namespace}:{patch.repo_name}")

    if failed_to_patch and args.output_failed is not None:
        print("\n".join(failed_to_patch), file=args.output_failed)
...
def generate_params_patches(
    ...
    maybe_patches = (
        generate_params_patch(repo, namespace_to_workspace_map) for repo in repos
    )
    return list(filter(None, maybe_patches))


def generate_params_patch(
    repo_obj: dict[str, Any], namespace_to_workspace_map: dict[str, str]
...
def refresh_patch(
    patch: RepositoryPatch, namespace_to_workspace_map: dict[str, str]
) -> RepositoryPatch | None:
    repo_json = kubectl(
        "get", "-n", patch.repo_namespace, "repository", patch.repo_name, "-o", "json"
    )
    return generate_params_patch(json.loads(repo_json), namespace_to_workspace_map)
